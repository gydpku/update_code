...
for iteration in range(1,5):
    ...
    if iteration==0:
        ...
        for i in range(iteration):
            ...
            previous_gradients.append([key for key in torch.load('instruct_baseline_gsm8k_3.1_doc_rate_0.1_0_ooa_instructions_data_t_3.pt').keys()])
#        pdb.set_trace()
...
#            pdb.set_trace()
    ...
    try:
        ...
        #    win_paths=[path[2] for path in paths_score if path[1]>=paths_score[-1][1]]
        if len(win_paths)>1:
            best_path,best_performance=bayesian_search(win_paths,exp_name=experiment_name,base_model_path=base_model_path,task=task_name,valid_data=valid_data)
        #    if iteration>0:
        #        win_paths_add=win_paths+global_best_group

        f_test,c_test=valid_results_collect(best_path, test_examples, task_name)
        avg_test_acc=len(c_test)/(len(c_test)+len(f_test))
        print(len(c_test)/(len(c_test)+len(f_test)),'avg_acc', iteration,'iteration')
        print("win_paths",win_paths)
        if iteration==0:
            global_best_performance=paths_score[-1][1]
        if best_performance>global_best_performance:
            global_best_performance=best_performance
            global_best_group=win_paths
            ooa_failed_cases, im_failed_cases, correct_cases=process_validation_batch_major(best_path, valid_data,task=task_name,seed=False, iteration=100)  
        print("Update!")
        torch.save(global_best_model_path,'global_best_model_path_{0}_{1}.pt'.format(experiment_name,iteration))
        torch.save(keys,'chosen_gradient_{0}_{1}.pt'.format(experiment_name,iteration))
