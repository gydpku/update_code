...
from auto_valid import data_self_valid
...
print('Starting')
experiment_name='baseline_auto_8'
output_path='/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/'
cur_path='/dccstor/obsidian_llm/yiduo/summary/src/'
model='LLama3-8B'
base_model_path='/dccstor/obsidian_llm/yiduo/llama-3-instruct' #'/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/MedNLI_auto_checkbaseline_auto_2_10001e-6_model' #'/dccstor/obsidian_llm/yiduo/llama-3-instruct' #'/dccstor/obsidian_llm/yiduo/h100_data/llama-3-8b'
task_name='MedNLI_auto_check_8' #'GSM8k'
train_seed=2024
task_instruction='The domain is Medical. The TASK: Please classify the relationship between the given premise and hypothesis into one of the following labels: entailment, contradiction, or neutral.' #task_instruction='You are given a word problem involving basic arithmetic, algebra, or geometry. Your task is to carefully read the problem and provide a step-by-step solution, ensuring that all intermediate steps are shown clearly.'

def load_task_dataset(dataset_name,valid_num=100):
    if 'mednli' in dataset_name.lower():
        test_examples=[]
        ...
        domain='Medical'
    if  'gsm8k' in dataset_name.lower():  #dataset_name=='GSM8k':
        test_examples=[]
...
#f_test,c_test=valid_results_collect(base_model_path, test_examples, task_name)
#print(len(c_test)/len(test_examples))
valid_acc=[0]
best_valid_acc=0
checked_data_dict,auto_valid_data=data_self_valid(task_instruction,valid_data[:5],estimated_valid_data_num=100,iteration=3)
...
print('start finding')
valid_data=auto_valid_data #valid_data[:5]
for data_num in [1000,2000,3000,4000,5000,6000,7000,8000,9000,10000]:
    ...
    data=data_sample_pattern(task_instruction,domain,data_num,store_name,'',demo_examples=valid_data,temperature=0.7,task_name=task_name,neg_sample=True,pattern=False,voting=False,iteration_number=1,sample_demo_num=3,passage_num=10000,valid_num=100,types=None)
