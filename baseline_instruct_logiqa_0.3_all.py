...
initial_model_path='/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/LogiQAbaseline_logiqa_15_20001e-6_model' #MedNLImednli_baseline_12_20001e-6_model' #GSM8kbaseline_random_2_20001e-6_model' #GSM8kgsm_baseline_10_20001e-6_model' #'/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/GSM8kbaseline_random_2_40001e-6_model'
initial_data_path='dataset_LogiQA_2000baseline_logiqa_15' #'dataset_MedNLI_2000mednli_baseline_12' #'dataset_GSM8k_2000baseline_random_2' #dataset_GSM8k_4000baseline_random_2' #'dataset_LogiQA_3000baseline_logiqa'
learning_rate=1e-6
# Configure logging
logging.basicConfig(
    filename="{0}.log".format(experiment_name), 
...
#            pdb.set_trace()
            ...
            for num in [int((rate/3)*len(global_best_data)),int((rate*2/3)*len(global_best_data)),int(rate*len(global_best_data))]:
...
#        model_paths=torch.load('win_paths_logi.pt')
        
        model_paths = [os.path.join(output_path, model_name+'_model') for model_name in names][:9]
        pdb.set_trace()
        ...
        torch.save(win_paths,'win_paths_logi.pt')
        '''
        best_path,best_performance=bayesian_search(model_paths,exp_name=experiment_name,base_model_path=base_model_path,task=task_name,valid_data=valid_data)
